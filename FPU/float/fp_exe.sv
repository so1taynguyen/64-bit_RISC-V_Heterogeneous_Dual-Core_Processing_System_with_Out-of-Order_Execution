...
module fp_exe (
    ...
    input fp_fdiv_out_type fp_fsqrt_o,
    output fp_fdiv_in_type fp_fsqrt_i,
  ...
  always_comb begin
    ...
    if (op.fdiv) begin
      ...
      fp_fsqrt_i.data1 = extend1;
      fp_fsqrt_i.data2 = extend2;
      fp_fsqrt_i.fmt = fmt;
      fp_fsqrt_i.rm = rm;
      fp_fsqrt_i.op = op;
      fp_fsqrt_i.class1 = class1;
      fp_fsqrt_i.class2 = class2;
    end
    else begin
      fp_fdiv_i.data1 = '0;
      fp_fdiv_i.data2 = '0;
    ...
    else if (fp_fdiv_o.ready) begin
      fp_rnd = fp_fdiv_o.fp_rnd;
    end 
    else if (fp_fsqrt_o.ready) begin
      fp_rnd = fp_fsqrt_o.fp_rnd;
    end
    else if (op.fcvt_f2f) begin
      fp_rnd = fp_cvt_f2f_o.fp_rnd;
    end 
    else if (op.fcvt_i2f) begin
    ...
    else if (op.fmadd | op.fmsub | op.fnmadd | op.fnmsub | op.fadd | op.fadd | op.fsub | op.fmul) begin
      ready = 0;
    end 
    else if (op.fdiv | op.fsqrt) begin
      ready = 0;
    end 
    else if (op.fcmp) begin
      result = fp_cmp_o.result;
      flags  = fp_cmp_o.flags;
    end 
    else if (op.fsgnj) begin
      result = fp_sgnj_o.result;
      flags  = 0;
    end 
    else if (op.fmax) begin
    ...
    else if (op.fclass) begin
      result = {54'h0, class1};
      flags  = 0;
    end 
    else if (op.fmv_f2i) begin
      result = data1;
      flags  = 0;
    end 
    else if (op.fmv_i2f) begin
      result = data1;
      flags  = 0;
    end 
    else if (op.fcvt_f2f) begin
